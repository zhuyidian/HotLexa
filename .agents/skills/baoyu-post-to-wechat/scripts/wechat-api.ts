...
function renderMarkdownWithPlaceholders(
  ...
  const runningWithBun = execBaseName === "bun" || execBaseName === "bun.exe";
  const command = runningWithBun ? process.execPath : (process.env.NPX_COMMAND || "npx");
  const args = runningWithBun
...
const FOLLOW_GUIDE_MARKER = "data-to-wechat-follow-guide";
const FOLLOW_GUIDE_PHRASE = "欢迎关注「一点IT+」";
const FOLLOW_GUIDE_HTML = [
  `<section ${FOLLOW_GUIDE_MARKER}="true" style="margin: 32px 0 8px; padding: 16px 18px; border: 1px solid #E8EEF5; border-radius: 12px; background: #F6F8FA; text-align: center;">`,
  `  <p style="margin: 0; color: #0F4C81; font-size: 16px; font-weight: 700; line-height: 1.8;">如果这篇文章对你有帮助，欢迎关注「一点IT+」。</p>`,
  `  <p style="margin: 6px 0 0; color: #57606a; font-size: 14px; line-height: 1.8;">我会持续分享 AI 工具、自动化实践和真实项目踩坑。</p>`,
  "</section>",
].join("\n");

function appendFollowGuide(html: string): string {
  const content = html.trim();
  if (!content) return FOLLOW_GUIDE_HTML;
  if (content.includes(FOLLOW_GUIDE_MARKER) || content.includes(FOLLOW_GUIDE_PHRASE)) {
    return content;
  }
  if (/<\/section>\s*$/i.test(content)) {
    return content.replace(/<\/section>\s*$/i, `${FOLLOW_GUIDE_HTML}\n</section>`);
  }
  return `${content}\n${FOLLOW_GUIDE_HTML}`;
}
...
async function main(): Promise<void> {
  ...
  htmlContent = appendFollowGuide(htmlContent);
  ...
  if (args.dryRun) {
    console.log(JSON.stringify({
      ...
      htmlPath,
      followGuide: htmlContent.includes(FOLLOW_GUIDE_MARKER),
  ...
  const rawCoverPath = args.cover ||
    ...
    frontmatter.cover ||
    frontmatter.image;
  const coverPath = rawCoverPath && !isHttpUrl(rawCoverPath) && !path.isAbsolute(rawCoverPath) && args.cover
